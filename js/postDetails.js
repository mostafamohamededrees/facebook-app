...
// GET DETAILS OF THE CLICKED POST //
...
function createPostHTML(data, commentsContent) {
  return `
...
async function addComment() {
  ...
  try {
    ...
    toggleComments();
    successAlert("The comment has been added successfully", "success");
  } catch (error) {
    let errorMessage = error.response?.data?.message ?? "An error occurred";
    if (errorMessage === "Unauthenticated.") {
      errorMessage = "Please login to add a comment.";
    }
    successAlert(errorMessage, "danger");
  } finally {
    loader(false);
  }
}
